...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (type === 'noaa_chart') {
      ...
      // NOAA ENC WMS endpoints — try in order, all return nautical chart imagery
      const urls = [
        // NOAA Chart Display Service (NCDS) — current primary, paper chart symbology
        // NOAA Chart Display Service — current primary WMS (paper chart symbology)
        'https://gis.charttools.noaa.gov/arcgis/rest/services/MarineChart_Services/NOAACharts/MapServer/exts/MaritimeChartService/WMSServer'
          + base + '&LAYERS=0,1,2,3,4,5,6,7',
        // NOAA ENC Online — ECDIS S-52 symbology fallback
